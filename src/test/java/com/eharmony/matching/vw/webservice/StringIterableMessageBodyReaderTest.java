...
public class StringIterableMessageBodyReaderTest {
	...
	/*
	 * Tests that spaces at the beginning don't mess things up. Also tries
	...
	public void spacesAtBeginningAndDifferentNewlinesTest()
		...
		for (String newLineToUse : new String[] { "\n", "\r", "\r\n" }) {
			...
			// Iterable<String> theIterable = toTest.readFrom(null, null, null,
	 ...
	 * (git@github.com:JohnLangford/vowpal_wabbit.git). I made a slight
	 * modification to the very first example in that file. I added the
	 * "vwExamples=" marker to the first example, because the
	 * PlainTextMessageBodyReader expects that the stream of examples
	 * will begin with this marker. The entire file was then saved to
	 * ner.train.withVwExamplesMarkerPrePended.gz.
	 */
	@Test
	public void hugeFileTest() throws IOException {
		...
		GZIPInputStream gzipInputStream = new GZIPInputStream(this
				.getClass()
				.getClassLoader()
				.getResourceAsStream(
						"ner.train.withVwExamplesMarkerPrePended.gz"));
		...
		// Iterable<String> theIterableOfExamples = toTest.readFrom(null, null,
		// null, mediaType, null, gzipInputStreamForTestSubject);
		//
		// //the first line in our test file contains 'vwExamples=', read it and
		// remove that marker.
		// //we have to to this because the iterable will return examples
		// without the 'vwExamples=' marker.
		...
		//
		// //TODO: get a jenkin's build going, turn on code coverage + findbugs
		// etc etc
		//
		...
		// }
		//
		// expectedExample = testReader.readLine(); //move to next expected
		// example
		// }
		//
		// Assert.assertTrue(testReader.readLine() == null); //ensure all
		// examples read and verified.
	}
